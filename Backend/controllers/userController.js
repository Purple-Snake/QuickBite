    ...
    try {
        ...
        bcrypt.hash(password, 10, function(err, hash) {
            ...
            const token = JWT.sign(
            {
                ...
                likedItems: createdUser.likedItems,
        ...
        const token = JWT.sign(
            {
                ...
                likedItems: existingUser.likedItems,
    ...
    try {
        ...
        JWT.verify(token, process.env.JWT_Secret, (err, decoded) => {
            ...
            if (decoded.role == "admin") {
                return res.status(200).json({ role: "admin", userName: decoded.userName, likedItems: decoded.likedItems  })
            } else {
                return res.status(200).json({ role: "user", userName: decoded.userName, likedItems: decoded.likedItems })
            }
        })
    } catch (error) {
        res.json(false)
